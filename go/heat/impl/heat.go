...
func NewState(nx int, nt int, threads int) *State {
	...
	var chunks = util.Chunks(initialGrid, getChunkSize(nx, threads))
	...
	if len(parts) != threads {
		panic(fmt.Sprintf("%d != %d", len(parts), threads))
	}
...
func getChunkSize(nx int, threads int) int {
	return int(math.Ceil(float64(nx) / float64(threads)))
}
