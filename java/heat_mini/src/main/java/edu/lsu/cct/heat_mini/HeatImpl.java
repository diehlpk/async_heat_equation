...
public class HeatImpl {
    ...
    public HeatImpl(int nx, int nt, int threads) {
        ...
        Channel[] leftGhosts  = makeGhostChannels(threads);
        Channel[] rightGhosts = makeGhostChannels(threads);

        var chunkSize = ((int) Math.ceil(((double)nx)/((double)threads)));
    ...
    private Channel[] makeGhostChannels(int threads) {
        var arr = new Channel[threads];
        for (int i = 0; i < arr.length; i++) {
            arr[i] = new Channel(channelBufferSize);
        }
        return arr;
    }
